...
public class RandomizedSingleMountainLevel extends Level {

	@Override
	protected List<Line> createLevel(int width, int height){
		List<Line> lines = new ArrayList<>();

		int leftGroundElevation = height - 20 - random.nextInt(50);
		int rightGroundElevation = height - 20 - random.nextInt(50);

		int platformWidth = LunarGame.PLATFORM_WIDTH;
		int platformHeight = 6;

		int mountainPeakX = width / 2 + random.nextInt(-30, 30) - 5;
		int mountainPeakY = height / 2 + random.nextInt(-100, 0) - 5;

		int leftMountainBaseX = width / 4 + random.nextInt(-50, 50);
		int leftMountainBaseY = leftGroundElevation;
		int rightMountainBaseX = 3 * width / 4 + random.nextInt(-50, 50);
